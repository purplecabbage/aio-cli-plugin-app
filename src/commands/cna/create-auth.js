...
class CNACreate extends Command {
  async run () {
    ...
    if (template != null) {
        let authRepoPath = path.resolve("./adobeio-cna-lib-auth-ims/")
        await cloneRepo()
        await installDeps(authRepoPath)
        await configureAuth(authRepoPath)
        await deployAuth(authRepoPath)
        //this.log(`Auth actions created successfully`)
        //console.log(config)
...
async function configureAuth (destDir) {
    return new Promise(function (resolve, reject) {
        // todo: apply name from flags to package.json
        ...
        cwd: destDir,
        stdio: 'inherit',
        env: {...process.env,"IMS_AUTH_TYPE":"code","IMS_AUTH_PERSIST":false}
        })
        child.on('error', err => {
            cli.action.stop('failed')
            console.log('error ' + err)
...
async function deployAuth (destDir) {
    return new Promise(function (resolve, reject) {
        ...
        const child = spawn('npm', ['run','deploy'], {
            cwd: destDir,
            stdio: 'inherit',
            env: {...process.env,"OW_NAMESPACE":config.get('runtime.namespace')}
        })
        child.on('error', err => {
            cli.action.stop('failed')
            console.log('error ' + err)
